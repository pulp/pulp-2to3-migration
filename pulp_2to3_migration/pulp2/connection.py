    ...
    try:
        ...
        if username:
            ...
            raise ConfigurationError(
                _(
                    "The config specified a database password, but is "
                    "missing a database username."
                )
            )

        # Wait until the Mongo database is available
        mongo_retry_timeout_seconds_generator = itertools.chain(
        ...
        if seeds != "":
            if len(seeds_list) > 1 and not replica_set:
                raise ConfigurationError(
                    _(
                        "Database 'replica_set' config must be specified "
            ...
            for m in cls._decorated_methods:
                try:
                    setattr(
                        instance,
                        m,
                        cls.retry_decorator(full_name)(getattr(instance, m)),
                    )
                except AttributeError:
                    pass
