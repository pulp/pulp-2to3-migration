...
CONTENT_COUNT = {"file": 3, "file2": 3, "file-large": 10, "file-many": 250}
...
class TestMigrationBehaviour(BaseTestFile, unittest.TestCase):
    ...
    def _test_pulp2content(self, plan):
        ...
        pulp2content = self.pulp2content_api.list(ordering="pulp2_id", limit=1).results[
            0
        ]
    ...
    def _test_pulp2repositories(self, plan):
        ...
        pulp3_pub = self.file_publication_api.read(
            pulp2repository.pulp3_publication_href
        )
        pulp3_dist = self.file_distribution_api.read(
            pulp2repository.pulp3_distribution_hrefs[0]
        )
        ...
        self.assertEqual(
            pulp3_repo.latest_version_href, pulp2repository.pulp3_repository_version
        )
        ...
        self.assertEqual(
            pulp3_pub.repository_version, pulp2repository.pulp3_repository_version
        )
    ...
    def test_many_into_one_repo(self):
        ...
        self.assertEqual(
            self.file_repo_versions_api.list(pulp3_repo.pulp_href).count, 4
        )
        ...
        for pulp2repo in pulp2repositories:
            with self.subTest(pulp2repo=pulp2repo):
                ...
                self.assertEqual(
                    repo_content.count, CONTENT_COUNT[pulp2repo.pulp2_repo_id]
                )
    ...
    def test_importer_different_repo(self):
        ...
        pulp2repositories = self.pulp2repositories_api.list(
            ordering="pulp2_repo_id"
        ).results
    ...
    def test_distributor_different_repo(self):
        ...
        pulp3_dist1 = self.file_distribution_api.read(
            pulp2repo1.pulp3_distribution_hrefs[0]
        )
        pulp3_dist2 = self.file_distribution_api.read(
            pulp2repo2.pulp3_distribution_hrefs[0]
        )
        ...
        self.assertTrue(pulp2repo2.is_migrated)
        self.assertEqual(
            pulp3_pub1.repository_version, pulp2repo1.pulp3_repository_version
        )
        self.assertEqual(
            pulp3_pub2.repository_version, pulp2repo2.pulp3_repository_version
        )
    ...
    def test_no_on_demand_importer(self):
        ...
        pulp3_repo = self.file_repo_api.list().results[0]
        repo_content = self.file_content_api.list(
            repository_version=pulp3_repo.latest_version_href
        )

        self.assertEqual(
            self.file_repo_versions_api.list(pulp3_repo.pulp_href).count, 1
        )
    ...
    def test_no_immediate_importer(self):
        ...
        self.assertEqual(
            self.file_repo_versions_api.list(pulp3_repo.pulp_href).count, 2
        )
