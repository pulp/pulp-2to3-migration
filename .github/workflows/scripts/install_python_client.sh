...
then
  ...
  docker exec pulp pip3 install pulp-2to3-migration-client==$VERSION
  mkdir -p dist
...
find . -name "*.whl" -exec docker exec pulp pip3 install /root/pulp-openapi-generator/pulp_2to3_migration-client/{} \;
...
tar cvf ../../pulp-2to3-migration/python-client-docs.tar ./docs
exit $?
