...
export VERSION=$(ls pulp_2to3_migration_client* | sed -rn 's/pulp_2to3_migration_client-(.*)\.gem/\1/p')
...
if [ "$response" == "200" ];
then
  echo "pulp-2to3-migration client $VERSION has already been released. Skipping."
  exit
fi
